...
2. A `bowtie` v0.12.5 executable must exist at the same path on all
   ...
   via the [`--bowtie`](#cb-hadoop-bowtie) option OR located in the directory specified in
   ...
   overridden by [`--bowtie`](#cb-hadoop-bowtie) or by defining `CROSSBOW_BOWTIE_HOME`.
...
see [`--intermediate`](#cb-local-intermediate) and [`--output`](#cb-local-output) options.
